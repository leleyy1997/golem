...
// 计算状态
function calculateStatus(remaining, total, settings) {
...
// 写入所有耗材数据
export function writeFilaments(filaments) {
    try {
        ensureDataDir();
        const data = {
            filaments,
...
// 添加耗材
export function addFilament(filament) {
    const filaments = readFilaments();
    const settings = readSettings();
    const newFilament = {
        ...filament,
